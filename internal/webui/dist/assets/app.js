...
const compact=n=>Number(n)>=1e6?(n/1e6).toFixed(2)+'M':Number(n)>=1000?(n/1000).toFixed(1)+'K':number(n);

const money=n=>'$'+Number(n||0).toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:Number(n)>0&&Number(n)<.01?6:4});

const dateTime=n=>n?new Date(n).toLocaleString('zh-CN',{month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit',hour12:false}):'—';

const ms=n=>Number(n)>=1000?(Number(n)/1000).toFixed(2)+' s':Math.round(Number(n)||0)+' ms';

const json=v=>JSON.stringify(v,null,2);

const uid=()=>crypto.randomUUID?.()||Math.random().toString(36).slice(2);

const pages=[['overview','总览','grid','WORKSPACE'],['providers','供应商','provider'],['models','模型库','model'],['routes','智能路由','route'],['playground','调试台','play'],['usage','额度与用量','usage','OBSERVABILITY'],['requests','请求记录','request'],['sessions','会话亲和','session'],['jobs','后台任务','job'],['keys','访问密钥','key','SYSTEM'],['settings','系统设置','settings']];

const state={
  page:'overview',version:'',config:null,data:null,range:'24h',paused:false,loading:false,routeID:'',routeDraft:null,simulation:null,requestPage:1,requestQ:'',requestStatus:'',modelQ:'',modelProtocol:'',providerFilter:'',playProtocol:'chat',playResult:null,playBusy:false,playSession:uid(),lastSync:0
};

let refreshTimer,dialogSubmit,formError,secretValue='',renderGeneration=0;
...
function btn(text,action,ico='',extra='',type=''){
...
function empty(title,desc,action='',label='',ico='model'){
...
function toast(text,error=false,requestID=''){
...
function check(label,name,value,help=''){
...
function showDialog(title,sub,body,onSubmit=null,save='保存配置',wide=false){
 ...
 $('#overlay-root').append(d);
  ...
  d.addEventListener('close',()=>{d.remove();secretValue='';dialogSubmit=null;});
...
function confirm(title,desc,yes='确认'){
...
async function save(action,params,form){
...
async function enterApp(){
  ...
  await loadPage();
  clearInterval(refreshTimer);
  refreshTimer=setInterval(()=>{if(document.visibilityState==='visible'&&!state.paused&&!state.loading&&!$('dialog')&&!['INPUT','TEXTAREA','SELECT'].includes(document.activeElement?.tagName)&&['overview','usage','requests','jobs','sessions'].includes(state.page))loadPage(false).catch(report);},10000);

}

function head(eyebrow,title,desc,tools=''){
...
async function loadPage(animated=true){
...
function requestRows(items,short=false){
...
function selectRow(label,help,name,value,options){
