...
export class RPCError extends Error {
  constructor(message, code, requestID, status) { super(message); this.code=code; this.requestID=requestID; this.status=status; }
}
let csrf='';
...
export async function rpc(action,params={},options={}) {
  ...
  try {
    ...
    let body;try{body=await res.json();}catch{throw new RPCError('服务器没有返回有效 JSON','INVALID_RESPONSE','',res.status);}
    if(!res.ok||!body.ok)throw new RPCError(body.error?.message||'请求失败',body.error?.code||'ERROR',body.request_id,res.status);
    return body.data;
  } catch(e) { if(e.name==='AbortError')throw new RPCError('请求已超时或取消；写操作可能已经生效，请刷新确认','TIMEOUT','',0);throw e; }
