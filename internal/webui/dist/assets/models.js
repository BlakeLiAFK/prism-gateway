...
import {loadModelUsage,modelUsageOf,modelUsagePanel,rangeSwitch,usageCell,usageHead} from './usage.js';
...
export function models(){
  ...
  if(state.modelFilters.sortUsage)filtered.sort((a,b)=>(modelUsageOf(b.id)?.requests||0)-(modelUsageOf(a.id)?.requests||0));
  ...
  +modelFilterBar()
  +`<section class="card">${!scoped.length?empty('让模型库准备就绪','先添加供应商，再同步或手动添加模型。供应商模型列表不会自动确认协议能力与计价。','add-model','添加第一个模型'):!filtered.length?`<div class="empty"><div class="empty-icon">${icon('model')}</div><h3>没有匹配的筛选条件</h3><p>放宽筛选条件，或清除当前筛选查看全部模型。</p>${btn('清除筛选','model-filter-clear','close','','ghost')}</div>`:`<div class="table-scroll"><table><thead><tr><th>模型 / 上游 ID</th><th>供应商</th><th>原生协议</th><th>能力</th><th>Input / Output · $/M</th><th>并发</th>${usageHead()}<th>路由</th><th>启用</th><th></th></tr></thead><tbody>${visible.map(m=>`<tr><td><div class="cell-title">${E(m.name||m.id)}${(state.data?.missing||[]).some(x=>x.id===m.id)?' <span class="off-tag" title="定期对照上游 /models 时没有找到这个模型，可能已下架；网关不会自动禁用">上游列表中找不到</span>':''}${(d=>d?` <span class="off-tag" title="上游公布 ${money(d.upstream_input)} / ${money(d.upstream_output)} 每百万 Token，本地为 ${money(d.input)} / ${money(d.output)}；网关不会自动改价">上游价格已变</span>`:'')((state.data?.price_drift||[]).find(x=>x.id===m.id))}</div><div class="cell-sub mono">${E(m.upstream)}</div></td><td class="small">${E(getProvider(m.provider_id)?.name||m.provider_id)}</td><td>${badge(m.protocol)}</td><td><div class="flex" style="gap:4px">${m.tools?badge('TOOLS'):''}${m.vision?badge('VISION'):''}${badge(compact(m.context_window))}</div></td><td class="mono">${!m.pricing_set?'未确认价格':!(m.input_price||m.output_price||m.cache_price||m.write_price)?'<span class="free-tag">免费</span>':`${money(m.input_price)} / ${money(m.output_price)}`}</td><td class="mono">${m.concurrency}${m.rpm?` <span class="tiny muted">/ ${m.rpm} RPM</span>`:''}</td><td>${usageCell(m)}</td><td>${routeCell(m.id)}</td><td><button class="switch ${m.enabled?'on':''}" role="switch" aria-checked="${m.enabled}" aria-label="启用 ${E(m.name)}" data-action="toggle-model" data-id="${E(m.id)}"></button></td><td><div class="table-actions"><button class="icon-btn" data-action="edit-model" data-id="${E(m.id)}" aria-label="编辑模型">${icon('edit')}</button><button class="icon-btn" data-action="delete-model" data-id="${E(m.id)}" aria-label="删除模型">${icon('trash')}</button></div></td></tr>`).join('')}</tbody></table></div>${truncated?`<div class="list-foot"><span>已显示前 ${visible.length} 个，共 ${filtered.length} 个匹配</span>${btn(`显示全部 ${filtered.length} 个`,'model-show-all','','','ghost small')}</div>`:''}`}</section>`;
...
function matchesFilters(m,f){
  ...
  if(f.used&&!modelUsageOf(m.id)?.requests)return false;
  return true;
...
export function modelFilterBar(){
  ...
  const active=!!(f.status||f.inRoute||f.unpriced||f.tools||f.vision||f.used);
  return `<div class="filter-bar"><div class="filter-group">${statusChip('','全部')}${statusChip('enabled','已启用')}${statusChip('disabled','未启用')}</div><div class="filter-group">${toggleChip('inRoute','在路由中')}${toggleChip('unpriced','计价未确认',true)}${toggleChip('tools','支持工具')}${toggleChip('vision','支持图像')}${toggleChip('used','有用量')}</div>${active?btn('清除筛选','model-filter-clear','close','','ghost small'):''}<div class="filter-range">${rangeSwitch()}</div></div>`;
