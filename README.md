...
**同步的新模型默认启用。** 同步会把上游声明的名称、上下文、输出上限、工具与图像能力、各项单价一并带过来；上游没声明的按常用值填：上下文 128K、输出上限 min(上下文, 64000)、支持工具、不支持图像、并发 4。有两项不会自动打开：**计价不算确认**，需到“模型库”核对后勾选“我已确认以下计价”，否则费用记为未知、不能设美元预算；**跨协议丢弃推理内容**需逐个模型显式开启。上游的声明只是它自己的说法，账户实际可用能力与费率仍以你的账单为准；不想被调用的模型在“模型库”关掉即可。内置少量 OpenCode Go 协议映射仅作为同步时的初始提示，不自动编造额度。

在“智能路由”创建 `auto-coding` 等入口，将模型加入候选池（勾选未启用的模型时，保存路由会一并启用）；拖动或用上下箭头排序，点击“应用排序”。原生兼容优先、会话亲和、本地预算压力与候选优先级决定选择。静态“模拟选择”不会产生模型调用费用。
...
| 三协议 | 原生转发；可选的逐模型推理内容丢弃（默认关闭，丢弃时响应头标注）；常用文本、图片引用、函数工具调用及结果的显式跨协议转换；SSE 转发和转换 |
