...
**同步的新模型默认禁用。** 同步会把上游声明的名称、上下文、输出上限、工具与图像能力、各项单价一并带过来，但 **计价不算确认**：到“模型库”核对协议与价格、勾选“我已确认以下计价”后再启用。上游的声明只是它自己的说法，账户实际可用能力与费率仍以你的账单为准。内置少量 OpenCode Go 协议映射仅作为同步时的初始提示；新型号、供应商行为及价格必须按实际账户确认，不自动编造额度。
