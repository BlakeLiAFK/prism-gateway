...
- 修复：重启后上游检查没有在第一轮重跑（清掉的上次运行时间只改了内存，被同一轮里先跑的任务从库里读了回来）。
- 修复：上游检查的发现如果当时推送失败（如通道未启用），之后不会再推。现在每条发现推送成功后才记入去重，
  失败的下次运行补推；下架提醒 30 天后重复一次，价格变动按「模型 + 上游价格」去重。

## 1.29.1 · 2026-09-26

- 告警推送新增「Webhook（POST 纯文本）」：`kind: "webhook_text"`，原样 POST 消息正文（`text/plain; charset=utf-8`），
  换行保留；所有推送的标题「【Prism Gateway】」独占一行，明细逐行列出。
- 定时任务有发现但推送通道未启用时，记为「未推送」而不是失败，不写入任务列表（此前额度预警每 10 分钟一条失败记录）。

## 1.29.0 · 2026-09-26

- 定时任务（系统设置 → 定时任务）：每项可单独开关、调参并「立即运行」，时区默认 UTC+8，每分钟检查一次。
  - 自动备份（默认关闭）：每天生成 `backups/gateway-auto-*.db`，只轮转自动备份，保留份数可调。
  - 额度预警：定时查询 Command Code、Z.AI 窗口额度，达到阈值（默认 80%）推送；同一窗口每个重置周期只推一次。
  - 每日日报（默认关闭）：昨日请求数、成功率、估算花费、常用模型、花费最多的 Key、失败最多的模型。
  - Key 到期提醒：到期前 N 天（默认 3 天）推送一次。
  - 数据库维护：每天 `PRAGMA optimize` 并截断 WAL。
  - 上游检查：原下架检测并入调度，新增价格变动检测（已确认价格与上游 `/models` 声明相差超过 1%），
    只提示不改价；模型库打「上游价格已变」标签，总览「需要关注」提示，新发现时推送。
- 推送都走「告警推送」配置的通道；运行记录与去重键存数据库，交接部署后不会重复执行或重复推送。
  产生动作或失败的运行写入任务列表，任务记录保留 30 天。
- 新增 `schedule.get`、`schedule.save`、`schedule.run`；`model.stats` 与 `dashboard.get` 新增 `price_drift`。

## 1.28.0 · 2026-09-26

- 路由实时输出速度改为流式片段到达即计入（约 3 字符 / token 估算），进行中的长回答也实时反映；
  没有流式输出时退回已完成请求的统计。`route.live` 新增 `live_tok_s`。
- 会话亲和列表只显示未过期的会话，并显示何时过期（`session.list` 返回 `expires_at`）。
- 转发时补上 `x-session-id`（客户端未带时用会话 ID），OpenRouter 据此把同一会话固定到同一家上游，缓存持续命中。
- 每 6 小时对照上游 `/models` 列表，已启用但列表里找不到的模型在总览「需要关注」与模型库中提示；
  只提示不自动禁用，列表拉取失败的供应商跳过。

## 1.27.0 · 2026-09-26

- 告警推送（系统设置 → 告警推送）：Telegram Bot 或 Webhook（POST JSON）。可选事件：上游额度窗口用完、
  同一模型连续 5 次失败、路由全部候选因容量或上游问题不可用。同一事件 30 分钟内只推一次，推送失败只记日志。
- Bot Token / Webhook 地址加密保存在数据库，接口只返回是否已设置，不进入配置快照与导出。
- 新增 `alert.get`、`alert.save`、`alert.test`。

## 1.26.0 · 2026-09-26

- 网关 Key 可设到期时间（到期后返回 401 `KEY_EXPIRED`）、每分钟请求数上限（`KEY_RPM_LIMIT`）
  与近 24 小时 / 近 30 天估算花费上限（`KEY_BUDGET_LIMIT`，按已结束请求统计并缓存 10 秒，可能略超）。
- 新增 `apikey.update`：创建后可修改名称、授权范围、到期与限额，修改立即生效；`apikey.create` 同样接受这些字段。
- 访问密钥页：列表显示到期时间、「已用 / 上限」与 RPM，过期的 Key 标为「已过期」；新增「编辑」。

## 1.25.0 · 2026-09-26

- 平滑停止与交接：收到停止信号后立即关闭监听、释放数据库锁，再等在途请求结束（「平滑停止等待」，默认 600 秒）；
  新进程启动时数据库锁被占用会最多等 60 秒接手。配合 systemd 两实例轮换与反代重试，部署不再切断进行中的请求。
- 空闲超时：供应商超时只限制等待响应头；流式输出过程中改为连续「流式空闲超时」（默认 120 秒）没有数据才断开，
  持续输出的长回答不再被总时长切断，卡死的上游也能及时释放并发名额。
- 准入被拒可观测：并发满、冷却、RPM、本地预算导致的拒绝计入实时面板（候选显示近 5 分钟被拒次数）；
  所有候选都被拒时写一条 rejected 请求记录，客户端收到 429 时请求日志里也能查到。
- 系统设置新增「流式空闲超时」「平滑停止等待」；供应商「超时」说明改为只管等待响应头。

## 1.24.0 · 2026-09-25

- 新增用量小时汇总表 `usage_hourly`（小时 + Key + 请求名 + 模型 + 供应商）：请求结束时单条累加，
  首次启动从历史请求回填，计价补录后重建受影响的小时，保留 400 天（请求明细仍按保留天数清理）。
- 模型库、供应商卡片的用量统计改读汇总表，不再扫描请求明细；统计只含已结束的请求，窗口按整点对齐。
- 访问密钥页：每把 Key 显示所选时间范围的请求数、成功率、tokens、估算花费；「用量」详情给出
  近 30 天每日请求、常用模型、常用路由与近 7 天来源（IP + 客户端）。
- `apikey.list` 附带 `usage`（参数 `days`）；新增 `key.usage`；requests 新增 (key_id, started_at) 索引。
