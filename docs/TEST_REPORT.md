...
## 4.5 并发与长稳压测（v1.7.1 起，PRJ-X09）

`scripts/loadtest.py` 用本地演示模型施压，不花钱也不依赖外部服务。
...
| 吞吐 | 154 rps |
| 成功率 | **100%**（全部 200） |
| 延迟 p50 / p95 / p99 | 356 / 891 / 1006 ms |
| RSS 起 / 峰 / 末 | 26.3 / 32.9 / 32.9 MB |
| 排空后在飞请求 | **0** |

内存在第一分钟达到稳态后不再增长（66s 时 32.7 MB，302s 时 32.9 MB），
排除泄漏。并发计数器完全释放——不释放会让网关逐渐「假满」直到重启。
延迟主要来自演示模型刻意的逐字输出，不代表上游速度。

**纯吞吐**：`/healthz` 每次调用都做一次数据库查询，32 并发下 **2424 rps**，
p50 9.5 ms，无错误。说明 SQLite 的全局串行化在这个量级不是瓶颈。

**容量提示**：46481 条请求记录占 24.4 MB，约每条 550 字节。
按每天 10 万请求、保留 90 天估算，数据库约 5 GB，需要相应规划磁盘。
...
- 跨协议响应带 `X-Prism-Dropped: reasoning` 与
  `X-Prism-Ignored: thinking,cache_control,metadata,output_config`
- 另测：Anthropic 工具调用单轮、工具结果回传、流式工具参数分片拼装，全部通过

实测输入规模可达 20 万 token（系统提示 + 101 个工具定义），
免费模型单轮需 5～34 秒。链路正确，速度受上游限制。

## 5. 尚未验证 / 不在交付承诺内

- 原生 Windows 构建；Windows 使用 WSL2 路线。
  (macOS arm64 已于 v1.1.0 验证；Docker/Linux 已于 v1.4.0 验证)
- 付费供应商凭证与计费行为。（OpenRouter 免费模型的真实云端调用已验证）
- 最新 Codex / Claude Code 全部功能的实机端到端验证，特别是签名推理、压缩、WebSocket、服务端工具或响应检索工作流。
- 独立安全审计、所有第三方 SDK 版本。（并发压测与内存稳定性已于 v1.7.1 验证）
